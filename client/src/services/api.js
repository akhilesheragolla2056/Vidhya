...
export const aiAPI = {
  ...
  summarize: (content, length = 'medium') => 
    api.post('/ai/summarize', { content, length }),
}
...
export const labAPI = {
  getExperiments: () => api.get('/labs'),
  getExperiment: (id) => api.get(`/labs/${id}`),
  saveProgress: (id, state) => api.post(`/labs/${id}/progress`, { state }),
  submitResults: (id, results) => api.post(`/labs/${id}/submit`, { results }),
}
